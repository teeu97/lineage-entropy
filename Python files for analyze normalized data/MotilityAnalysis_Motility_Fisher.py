...
contingency_table = [[] for state in range(2)]
state_history_table = [[] for state in range(2)]
for state in range(2):
    new_table = [0, 0]
    added = False
    for row, motif in df.iterrows():
        if motif['State History'][2] == state+1:
            new_table[motif['State History'][3] - 1] += motif['Number of Lineages']
            added = True
        if motif['State History'][3] == 2 and added and all(new_table):
            contingency_table[state].append(new_table)
            state_history_table[state].append(motif['State History'][:3])
            new_table = [0, 0]
            added = False

with open('StateHistory_FisherExactTest_Motility.csv', mode='w') as file:
    ...
    for state in range(2):
        for ix in range(len(contingency_table[state])-1):
            for iy in range(ix+1, len(contingency_table[state])):
                writer_.writerow(['State History', 'State 1', 'State 2'])
                writer_.writerow([state_history_table[state][ix]] + [member for member in contingency_table[state][ix]])
                writer_.writerow([state_history_table[state][iy]] + [member for member in contingency_table[state][iy]])
                res = stats_.fisher_test(np.array([contingency_table[state][ix], contingency_table[state][iy]]))
                writer_.writerow(['p-value', res[0][0]])
                writer_.writerow([])
