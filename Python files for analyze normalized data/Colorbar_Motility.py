...
color_scalarMap = matplotlib.cm.ScalarMappable(
...
plt.figure()
plt.axis('off')
cbar = plt.colorbar(color_scalarMap, orientation='horizontal', ticks=[0, len(all_barcode_list) // 2, len(all_barcode_list)])
cbar.ax.set_xticklabels(['Lowest Motility', 'Medium Motility', 'Highest Motility'])
plt.savefig('Colorbar_Motility_Horizontal.svg', dpi=720, format='svg', bbox_inches='tight')
plt.close()

plt.figure()
plt.axis('off')
cbar = plt.colorbar(color_scalarMap, orientation='vertical', ticks=[0, len(all_barcode_list) // 2, len(all_barcode_list)])
cbar.ax.set_yticklabels(['Lowest Motility', 'Medium Motility', 'Highest Motility'])
plt.savefig('Colorbar_Motility_Vertical.svg', dpi=720, format='svg', bbox_inches='tight')
plt.close()
