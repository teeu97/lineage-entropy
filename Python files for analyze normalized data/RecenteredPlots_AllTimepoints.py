    ...
    for j in range(3):
        ...
        for barcode in all_barcode_list:
            ...
            if assigned_state_list[i] == j:
                ...
                state_length[j] += 1
for j in range(3):
    axs[j].set_title('State ' + str(j+1) +  ' (' + str(state_length[j]) + ')')
    axs[j].axis('off')
...
fig.savefig("RecenteredPlots_AllTimepoints.svg", bbox_inches=0, format='svg', dpi=720)