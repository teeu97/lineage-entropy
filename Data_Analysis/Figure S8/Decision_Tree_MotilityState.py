"""
Decision_Tree_MotilityState.py makes a decision tree that follows lineage based on their motility state (changing state
or not) across different timepoints.
"""
import pickle
import math
import numpy as np
...
for barcode, row in true_number_table.iterrows():
    ...
    if len(barcode_summary['cartesian_coord']) == 5:
        for i in range(4):
            ...
            if barcode_summary['assigned_state'][i] != barcode_summary['assigned_state'][i + 1]:
                barcode_summary['assigned_motility_state'].append(1)
            else:
                barcode_summary['assigned_motility_state'].append(0)
...
G = nx.balanced_tree(2, 4)  # create a balanced tree with 4 layers where each node has 2 outgoing edges
