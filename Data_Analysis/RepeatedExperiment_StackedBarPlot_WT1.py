...
for barcode, row in true_number_table.iterrows():
    new_list = [1, sum(row[1:4]), sum(row[17:20]), sum(row[41:44]), sum(row[57:60]), sum(row[33:36])]
...
plt.savefig('RepeatedExperiment_StackedBarPlot_WT1.svg', bbox_inches='tight', format='svg', dpi=720)
