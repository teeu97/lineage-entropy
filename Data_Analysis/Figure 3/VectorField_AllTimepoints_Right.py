...
def vector_field_size_weight_shifted_size_all(all_barcode_list):
    ...
    plt.arrow(6.5, 3, 5/6, 0, shape='full', head_width=0.1, color='black', linewidth=math.sqrt(default_size ** (0)), length_includes_head=True)
    plt.arrow(6.5, 2.5, 7/6, 0, shape='full', head_width=0.1, color='black', linewidth=math.sqrt(default_size ** (0)), length_includes_head=True)
    ...
    plt.axis('off')
    plt.legend(custom_lines, ['[0, 10)', '[10, 100)', '[100, 1000)', '[1000, 10000)]'], title='Number of Distinct Barcodes', loc='lower right', fontsize='small', framealpha=1, edgecolor='white')
    cbaxes = fig.add_axes([0.95, 3/22.5, 0.05, 0.65])
    cbar = plt.colorbar(color_scalarMap, pad=0.05, shrink=0.8, cax=cbaxes)
    # cbar.set_label('$\log_{10} Lineage Size$', rotation=270, labelpad=10)
    plt.savefig("VectorField_AllTimepoints_Right.svg", bbox_inches='tight', format='svg', dpi=720)

vector_field_size_weight_shifted_size_all(all_barcode_list)